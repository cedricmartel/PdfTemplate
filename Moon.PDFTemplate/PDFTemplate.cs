...
namespace Moon.PDFTemplate
{
	...
	public abstract class PDFTemplate
	{
		...
		/// Document font attrs 
		/// </summary>
		protected XmlAttributeCollection DefaultFontAttrs {
		...
		/// <returns></returns>
		protected TextBox _buildTextBox(XmlNode node, XmlAttributeCollection fontAttrs)
		{
		...
		/// <param name="fontAttrs"></param>
		/// <returns></returns>
		protected DrawElement _buildCustomElement(XmlNode node, XmlAttributeCollection fontAttrs){
		...
		/// <param name="fontAttrs"></param>
		/// <returns></returns>
		protected Image _buildImage(XmlNode node, XmlAttributeCollection fontAttrs)
		{
			//NOT support format.
			List<Variable> vars = new List<Variable>();
		...
		private void _buildDrawElement(Row row, XmlNode node, XmlAttributeCollection font)
		{
			foreach (XmlNode child in node.ChildNodes)
			{
				...
				{
					case "textbox":
						row.AddDrawElement(_buildTextBox(child, font));
						break;
					case "line":
						row.AddDrawElement(_buildLine(child, font));
						break;
					case "image":
						row.AddDrawElement(_buildImage(child, font));
						break;
					case "textblock":
					...
					case CustomElementConstant:
						row.AddDrawElement(_buildCustomElement(child, font));
						break;
				}
		...
		/// <param name="font"></param>
		/// <returns></returns>
		private Row _buildRow(XmlNode node, XmlAttributeCollection font)
		{
		...
		/// <returns></returns>
		private RowGroup _buildRowGroup(XmlNode node)
		{
		...
		private void _drawRow(List<DrawElement> drawElements, IDictionary data)
		{
		...
		protected void _drawRowGroup(RowGroup rowGroup, System.Collections.IDictionary data, DocumentGroup dGroup)
		{
			bool absolute = false;
			...
			foreach (Row row in rowGroup.Rows)
			{
				...
				_drawRow(row.DrawElements, data);
			}
		...
		protected void _drawHeader()
		{
			...
			foreach (Row row in rowGroup.Rows)
			{
				...
				_drawRow(row.DrawElements, headerData);
			}
		}
		...
		protected void _drawLoop(List<Hashtable> loopData)
		{
			...
			{
				_drawRowGroup(rowGroup, data, DocumentGroup.Loop);
			}
		}
...
//			}
			...
			_drawRowGroup(rowGroup, footerData, DocumentGroup.Footer);
			...
			Hashtable footerData)
		{
			...
			_drawRowGroup(PageDefinition.Body, bodyData, DocumentGroup.Body);
			_drawFooter();
		...
		protected void NextPage()
		{
			...
			if(PageDefinition.Footer is FooterGroup && ((FooterGroup)PageDefinition.Footer).Absolute){
				...
				_drawRowGroup(PageDefinition.Footer, footerData, DocumentGroup.Footer);
			}
